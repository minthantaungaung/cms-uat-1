...
namespace cms_api.Controllers
{
    [ApiController]
    [ApiVersion("1.0")]
    [Route("/v{version:apiVersion}/death")]
    public class DeathController : BaseController
    {
        ...
        [ProducesResponseType(typeof(ResponseModel<PagedList<DeathResponse>>), 200)]
        ...
        {
            var response = await deathRepository.List(page ?? 1, size ?? 10, name);
            return Ok(response);
        }
        #endregion
        ...
        public async Task<IActionResult> Create([FromForm] CreateDeathRequest model)
        {
            var response = await deathRepository.Create(model);
            return Ok(response);
        }
        #endregion
        ...
        [HttpGet("{id}")]
        [Authorize(AuthenticationSchemes = DefaultConstants.AccessTokenBearer)]
        [ProducesResponseType(typeof(ResponseModel<DeathResponse>), 200)]
        public async Task<IActionResult> Get([Required][FromRoute] Guid? id)
        {
            var response = await deathRepository.Get(id ?? Guid.Empty);
            return Ok(response);
        }
        #endregion
        ...
        [Authorize(AuthenticationSchemes = DefaultConstants.AccessTokenBearer)]
        [ProducesResponseType(typeof(ResponseModel<string>), 200)]
        public async Task<IActionResult> Update([FromForm] UpdateDeathRequest model)
        {
            var response = await deathRepository.Update(model);
            return Ok(response);
        }
        #endregion
        ...
        {
            var response = await deathRepository.ChangeStatus(model);
            return Ok(response);
        }
        #endregion
        ...
        {
            var response = await deathRepository.Delete(id);
            return Ok(response);
        }
        #endregion
    }
