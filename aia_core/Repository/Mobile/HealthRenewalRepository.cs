...
{
    public interface IHealthRenewalRepository
    {
        Task<ResponseModel<ServicingResponseModel>> Submit(ServiceCommonPartOne model);
    }
    public class HealthRenewalRepository : BaseRepository, IHealthRenewalRepository
    {
        #region "const"
        ...
        public HealthRenewalRepository(IHttpContextAccessor httpContext, IAzureStorageService azureStorage, IErrorCodeProvider errorCodeProvider,
        ...
        public async Task<ResponseModel<ServicingResponseModel>> Submit(ServiceCommonPartOne model)
        {
            try
            {
                ...
                {
                    ServiceHealthRenewal entity = new ServiceHealthRenewal();
                    ...
                    unitOfWork.GetRepository<Entities.ServiceHealthRenewal>().Add(entity);
                    ...
                    Console.WriteLine("Before CreateCaseToCRM");
                    //Task.Run(() => RunBackgroundCodeAsync(entity, client, policy, model,memberID,ServicingEmail));
                    await CreateCaseToCRM(client, entity, policy);
                    Console.WriteLine("After CreateCaseToCRM");
                    ...
                    try
                    {
                        ...
                        _ = notificationService.SendServicingNoti(memberID ?? new Guid(), entity.ID, EnumServicingStatus.Received, EnumServiceType.HealthRenewal, model.PolicyNo);
            ...
            {
                Console.WriteLine($"HealthRenewalRepository Submit Error | Ex message : {ex.Message} | Exception {ex}");
                    ...
                    try
                    {
                        ...
                        try
                        {
                            unitOfWork.GetRepository<Entities.ServiceMainDoc>().Add(new ServiceMainDoc
                            {
                                ...
                                FormId = GetServicingFormId(EnumServiceType.HealthRenewal),
                                DocName = doc,
                                DocType = format,
        ...
        public async Task<string> CreateCaseToCRM(Client client, ServiceHealthRenewal entity, Policy policy)
        {
            try
            {
                ...
                crmModel.RequestInfo.CaseCategory = "CC018";
        ...
        private async Task<byte[]> GenerateServicingPdf(ServiceHealthRenewal data, string pdfFileName, string holderName, string policyNumber, string policyStatus, string clientNo)
        {
            var path = Path.Combine(
            ...
            htmlData = htmlData.Replace("{{Title}}", "Request for Health Renewal");
            htmlData = htmlData.Replace("{{HolderName}}", holderName);
            htmlData = htmlData.Replace("{{ClientNo}}", clientNo);

            htmlData = htmlData.Replace("{{ChangeType}}", "Health Renewal");
        ...
        private async Task<string> UpadeGeneratePdfToCMS(byte[] pdfData, string policyNo, string fileName, ServiceHealthRenewal model)
        {
            var upload = new UploadBase64Request();
            ...
            try
            {
                ...
                Console.WriteLine($"HealthRenewal => azureStorage.UploadBase64Async => {JsonConvert.SerializeObject(azureUpload)}");
                ...
                await unitOfWork.GetRepository<Entities.ServiceMainDoc>().AddAsync(new ServiceMainDoc
                {
                    ...
                    CmsResponse = JsonConvert.SerializeObject(uploadResult),
                    FormId = GetServicingFormId(EnumServiceType.HealthRenewal, true),
                    DocName = fileName,
                    DocType = "pdf",
                    UploadStatus = uploadResult?.msg,
                });
