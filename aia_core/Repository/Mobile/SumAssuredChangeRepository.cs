...
{
    public interface ISumAssuredChangeRepository
    {
        Task<ResponseModel<ServicingResponseModel>> Submit(ServiceCommonPartOne model);
    }
    public class SumAssuredChangeRepository : BaseRepository, ISumAssuredChangeRepository
    {
        #region "const"
        ...
        public SumAssuredChangeRepository(IHttpContextAccessor httpContext, IAzureStorageService azureStorage, IErrorCodeProvider errorCodeProvider,
        ...
        public async Task<ResponseModel<ServicingResponseModel>> Submit(ServiceCommonPartOne model)
        {
            try
            {
                ...
                {
                    ServiceSumAssuredChange entity = new ServiceSumAssuredChange();
                    ...
                    unitOfWork.GetRepository<Entities.ServiceSumAssuredChange>().Add(entity);
                    ...
                    ILSumAssuredRequest iLSumAssuredRequest = new ILSumAssuredRequest();
                    iLSumAssuredRequest.newSumAssured = model.Amount;
                    iLSumAssuredRequest.policyNumber = model.PolicyNo;
                    iLSumAssuredRequest.requestId = entity.ID.ToString();

                    if(crmResponse.Code== "200")
                    {
                        iLSumAssuredRequest.incidentID = crmResponse.Data;  
                    }
                    else
                    {
                        iLSumAssuredRequest.incidentID = "";
                    }
                    
                    entity.ILRequest = JsonConvert.SerializeObject(iLSumAssuredRequest);
                    entity.ILRequestOn = Utils.GetDefaultDate();

                    //AIA IL
                    CommonRegisterResponse ilResponse = await aiaILApiService.SumAssuredSubmission(iLSumAssuredRequest);

                    entity.ILResponse = JsonConvert.SerializeObject(ilResponse);
                    entity.ILResponseOn = Utils.GetDefaultDate();
                    unitOfWork.GetRepository<Entities.ServiceSumAssuredChange>().Update(entity);
                    unitOfWork.SaveChanges();
                    ...
                    try
                    {
                        ...
                        _ = notificationService.SendServicingNoti(memberID ?? new Guid(), entity.ID, EnumServicingStatus.Received, EnumServiceType.SumAssuredChange, model.PolicyNo);
        ...
        private async Task RunBackgroundCodeAsync(ServiceSumAssuredChange entity, Client client, Policy policy, ServiceCommonPartOne model, Guid? memberID, string ServicingEmail, string pdfFileName, byte[] pdfData, CommonRegisterResponse ilResponse)
        {
            ...
            try
            {
                ...
                string subject = $"{entity.PolicyNumber}/Sum Assured Change/{client.Name}";
        ...
        public async Task<(List<byte[]>, List<string>)> UploadDocToCMS(ServiceCommonPartOne model, Policy policy, ServiceSumAssuredChange entity)
        {
            try
            {
                ...
                foreach (var doc in model.DocNameList)
                {
                    try
                    {
                        ...
                        ServiceSumAssuredChangeDoc docData = new ServiceSumAssuredChangeDoc();

                        docData.ServiceSumAssuredChangeID = entity.ID;
                        ...
                        unitOfWork.GetRepository<Entities.ServiceSumAssuredChangeDoc>().Add(docData);
                        unitOfWork.SaveChanges();
                        ...
                        try
                        {
                            unitOfWork.GetRepository<Entities.ServiceMainDoc>().Add(new ServiceMainDoc
                            {
                                ...
                                FormId = GetServicingFormId(EnumServiceType.SumAssuredChange),
                                DocName = doc,
                                DocType = format,
                    ...
                    {
                        MobileErrorLog($"ServiceSumAssuredChange => UploadBase64", ex.Message, JsonConvert.SerializeObject(ex), httpContext?.HttpContext.Request.Path);
                    }
        ...
        public async Task<CaseResponse> CreateCaseToCRM(Client client, ServiceSumAssuredChange entity, Policy policy)
        {
            try
            {
                ServiceSumAssuredChange data = unitOfWork.GetRepository<ServiceSumAssuredChange>().Query(x=> x.ID == entity.ID).FirstOrDefault();
                ...
                crmModel.RequestInfo.CaseCategory = "CC031";
                ...
                MobileErrorLog("ServiceSumAssuredChange aiaCrmApiService => CreateCase", "Request"
                    , JsonConvert.SerializeObject(crmModel), "v1/claim/claim");

                CaseResponse crmResponse = await aiaCrmApiService.CreateCase(crmModel);


                MobileErrorLog("ServiceSumAssuredChange aiaCrmApiService => CreateCase", "Response"
                ...
                {
                    MobileErrorLog("update ServiceSumAssuredChange tran ex after Crm call", ex.Message
        ...
        private async Task<byte[]> GenerateServicingPdf(ServiceSumAssuredChange data, string pdfFileName, string holderName, string policyNumber, string policyStatus, string clientNo)
        {
            var path = Path.Combine(
            ...
            htmlData = htmlData.Replace("{{Title}}", "Request for SumAssured Change");
            htmlData = htmlData.Replace("{{HolderName}}", holderName);
            htmlData = htmlData.Replace("{{ClientNo}}", clientNo);

            htmlData = htmlData.Replace("{{ChangeType}}", "Sum Assured Change");
        ...
        private async Task<string> UpadeGeneratePdfToCMS(byte[] pdfData, string policyNo, string fileName, ServiceSumAssuredChange model)
        {
            var upload = new UploadBase64Request();
            ...
            try
            {
                ...
                Console.WriteLine($"SumAssuredChange => azureStorage.UploadBase64Async => {JsonConvert.SerializeObject(azureUpload)}");
                ...
                await unitOfWork.GetRepository<Entities.ServiceMainDoc>().AddAsync(new ServiceMainDoc
                {
                    ...
                    CmsResponse = JsonConvert.SerializeObject(uploadResult),
                    FormId = GetServicingFormId(EnumServiceType.SumAssuredChange, true),
                    DocName = fileName,
                    DocType = "pdf",
                    UploadStatus = uploadResult?.msg,
                });
