...
{
    public interface IPolicySurrenderRepository
    {
        Task<ResponseModel<ServicingResponseModel>> Submit(ServiceWithdrawRequest model);
    }
    public class PolicySurrenderRepository : BaseRepository, IPolicySurrenderRepository
    {
        #region "const"
        ...
        public PolicySurrenderRepository(IHttpContextAccessor httpContext, IAzureStorageService azureStorage, IErrorCodeProvider errorCodeProvider,
        ...
        public async Task<ResponseModel<ServicingResponseModel>> Submit(ServiceWithdrawRequest model)
        {
            try
            {
                ...
                {
                    ServicePolicySurrender entity = new ServicePolicySurrender();
                    ...
                    unitOfWork.GetRepository<Entities.ServicePolicySurrender>().Add(entity);
                    ...
                    try
                    {
                        ...
                        _ = notificationService.SendServicingNoti(memberID ?? new Guid(), entity.ID, EnumServicingStatus.Received, EnumServiceType.PolicySurrender, model.PolicyNo);
            ...
            {
                MobileErrorLog("PolicySurrender Ex =>", ex.Message, JsonConvert.SerializeObject(ex), httpContext?.HttpContext.Request.Path);

                return errorCodeProvider.GetResponseModel<ServicingResponseModel>(ErrorCode.E500);
            }
        }

        private async Task RunBackgroundCodeAsync(ServicePolicySurrender entity, Client client, Policy policy, ServiceWithdrawRequest model, Guid? memberID, string ServicingEmail, string pdfFileName, byte[] pdfData)
        {
            ...
            try
            {
                ...
                MobileErrorLog("totalSize PolicySurrender", totalSize.ToString(), "", httpContext?.HttpContext.Request.Path);
                ...
                MobileErrorLog("attachments Count PolicySurrender", attachments.Count.ToString(), "", httpContext?.HttpContext.Request.Path);


                string subject = $"{entity.PolicyNumber}/Policy Surrender/{client.Name}";
        ...
        public async Task<(List<byte[]>, List<string>)> UploadDocToCMS(ServiceWithdrawRequest model, Policy policy, ServicePolicySurrender entity)
        {
            try
            {
                ...
                foreach (var doc in model.DocNameList)
                {
                    try
                    {
                        ...
                        ServicePolicySurrenderDoc docData = new ServicePolicySurrenderDoc();

                        docData.ServicePolicySurrenderID = entity.ID;
                        ...
                        unitOfWork.GetRepository<Entities.ServicePolicySurrenderDoc>().Add(docData);
                        unitOfWork.SaveChanges();
                        ...
                        try
                        {
                            unitOfWork.GetRepository<Entities.ServiceMainDoc>().Add(new ServiceMainDoc
                            {
                                ...
                                FormId = GetServicingFormId(EnumServiceType.PolicySurrender),
                                DocName = doc,
                                DocType = format,
                    ...
                    {
                        MobileErrorLog($"ServicePolicySurrender => UploadBase64", ex.Message, JsonConvert.SerializeObject(ex), httpContext?.HttpContext.Request.Path);
                    }
        ...
        public async Task<string> CreateCaseToCRM(Client client, ServicePolicySurrender entity, Policy policy)
        {
            try
            {
                ServicePolicySurrender data = unitOfWork.GetRepository<ServicePolicySurrender>().Query(x=> x.ID == entity.ID).FirstOrDefault();
                ...
                crmModel.RequestInfo.CaseCategory = "CC001";
                ...
                MobileErrorLog("ServicePolicySurrender aiaCrmApiService => CreateCase", "Request"
                    , JsonConvert.SerializeObject(crmModel), "v1/claim/claim");

                var crmResponse = await aiaCrmApiService.CreateCase(crmModel);


                MobileErrorLog("ServicePolicySurrender aiaCrmApiService => CreateCase", "Response"
                ...
                {
                    MobileErrorLog("update ServicePolicySurrender tran ex after Crm call", ex.Message
        ...
        private async Task<byte[]> GenerateServicingPdf(ServicePolicySurrender data, string pdfFileName, string holderName, string policyNumber, string policyStatus, string clientNo)
        {
            var path = Path.Combine(
            ...
            htmlData = htmlData.Replace("{{Title}}", "Request for Policy Surrender");
            htmlData = htmlData.Replace("{{HolderName}}", holderName);
            htmlData = htmlData.Replace("{{ClientNo}}", clientNo);

            htmlData = htmlData.Replace("{{ChangeType}}", "Policy Surrender");
        ...
        private async Task<string> UpadeGeneratePdfToCMS(byte[] pdfData, string policyNo, string fileName, ServicePolicySurrender model)
        {
            var upload = new UploadBase64Request();
            ...
            try
            {
                ...
                Console.WriteLine($"PolicySurrender => azureStorage.UploadBase64Async => {JsonConvert.SerializeObject(azureUpload)}");
                ...
                await unitOfWork.GetRepository<Entities.ServiceMainDoc>().AddAsync(new ServiceMainDoc
                {
                    ...
                    CmsResponse = JsonConvert.SerializeObject(uploadResult),
                    FormId = GetServicingFormId(EnumServiceType.PolicySurrender, true),
                    DocName = fileName,
                    DocType = "pdf",
                    UploadStatus = uploadResult?.msg,
                });
