...
{
    public interface IPartialWithdrawRepository
    {
        Task<ResponseModel<ServicingResponseModel>> Submit(ServiceWithdrawRequest model);
    }
    public class PartialWithdrawRepository : BaseRepository, IPartialWithdrawRepository
    {
        #region "const"
        ...
        public PartialWithdrawRepository(IHttpContextAccessor httpContext, IAzureStorageService azureStorage, IErrorCodeProvider errorCodeProvider,
        ...
        public async Task<ResponseModel<ServicingResponseModel>> Submit(ServiceWithdrawRequest model)
        {
            try
            {
                ...
                {
                    ServicePartialWithdraw entity = new ServicePartialWithdraw();
                    ...
                    unitOfWork.GetRepository<Entities.ServicePartialWithdraw>().Add(entity);
                    ...
                    serviceMain.ServiceType = EnumServiceType.PartialWithdraw.ToString();
                    ...
                    try
                    {
                        ...
                        _ = notificationService.SendServicingNoti(memberID ?? new Guid(), entity.ID, EnumServicingStatus.Received, EnumServiceType.PartialWithdraw, model.PolicyNo);
            ...
            {
                MobileErrorLog("PartialWithdrawal Ex => ", ex.Message, JsonConvert.SerializeObject(ex), httpContext?.HttpContext.Request.Path);

                return errorCodeProvider.GetResponseModel<ServicingResponseModel>(ErrorCode.E500);
            }
        }

        private async Task RunBackgroundCodeAsync(ServicePartialWithdraw entity, Client client, Policy policy, ServiceWithdrawRequest model, Guid? memberID, string ServicingEmail, string pdfFileName, byte[] pdfData)
        {
            ...
            try
            {
                ...
                string subject = $"{entity.PolicyNumber}/Partial Withdraw/{client.Name}";
        ...
        public async Task<(List<byte[]>, List<string>)> UploadDocToCMS(ServiceWithdrawRequest model, Policy policy, ServicePartialWithdraw entity)
        {
            try
            {
                ...
                foreach (var doc in model.DocNameList)
                {
                    try
                    {
                        ...
                        ServicePartialWithdrawDoc docData = new ServicePartialWithdrawDoc();

                        docData.ServicePartialWithdrawID = entity.ID;
                        ...
                        unitOfWork.GetRepository<Entities.ServicePartialWithdrawDoc>().Add(docData);
                        unitOfWork.SaveChanges();
                        ...
                        try
                        {
                            unitOfWork.GetRepository<Entities.ServiceMainDoc>().Add(new ServiceMainDoc
                            {
                                ...
                                FormId = GetServicingFormId(EnumServiceType.PartialWithdraw),
                                DocName = doc,
                                DocType = format,
        ...
        public async Task<string> CreateCaseToCRM(Client client, ServicePartialWithdraw entity, Policy policy)
        {
            try
            {
                ServicePartialWithdraw data = unitOfWork.GetRepository<ServicePartialWithdraw>().Query(x=> x.ID == entity.ID).FirstOrDefault();
                ...
                crmModel.RequestInfo.CaseCategory = "CC030";
                ...
                MobileErrorLog("ServicePartialWithdraw aiaCrmApiService => CreateCase", "Request"
                    , JsonConvert.SerializeObject(crmModel), "v1/claim/claim");

                var crmResponse = await aiaCrmApiService.CreateCase(crmModel);


                MobileErrorLog("ServicePartialWithdraw aiaCrmApiService => CreateCase", "Response"
                ...
                {
                    MobileErrorLog("update ServicePartialWithdraw tran ex after Crm call", ex.Message
        ...
        private async Task<byte[]> GenerateServicingPdf(ServicePartialWithdraw data, string pdfFileName, string holderName, string policyNumber, string policyStatus, string clientNo)
        {
            var path = Path.Combine(
            ...
            htmlData = htmlData.Replace("{{Title}}", "Request for Partial Withdraw");
            htmlData = htmlData.Replace("{{HolderName}}", holderName);
            htmlData = htmlData.Replace("{{ClientNo}}", clientNo);

            htmlData = htmlData.Replace("{{ChangeType}}", "Partial Withdraw");
        ...
        private async Task<string> UpadeGeneratePdfToCMS(byte[] pdfData, string policyNo, string fileName, ServicePartialWithdraw model)
        {
            var upload = new UploadBase64Request();
            ...
            try
            {
                ...
                Console.WriteLine($"Partial => azureStorage.UploadBase64Async => {JsonConvert.SerializeObject(azureUpload)}");
                ...
                await unitOfWork.GetRepository<Entities.ServiceMainDoc>().AddAsync(new ServiceMainDoc
                {
                    ...
                    CmsResponse = JsonConvert.SerializeObject(uploadResult),
                    FormId = GetServicingFormId(EnumServiceType.PartialWithdraw, true),
                    DocName = fileName,
                    DocType = "pdf",
                    UploadStatus = uploadResult?.msg,
                });
