...
namespace aia_core.Repository.Mobile
{
    ...
    {
        Task<ResponseModel<ServicingResponseModel>> Submit(PaymentFrequencyRequest model);
    }
    public class PaymentFrequencyRepository : BaseRepository, IPaymentFrequencyRepository
    {
        #region "const"
        ...
        public PaymentFrequencyRepository(IHttpContextAccessor httpContext, IAzureStorageService azureStorage, IErrorCodeProvider errorCodeProvider,
        ...
        public async Task<ResponseModel<ServicingResponseModel>> Submit(PaymentFrequencyRequest model)
        {
            try
            {
                ...
                {
                    ServicePaymentFrequency entity = new ServicePaymentFrequency();
                    entity.ID = Guid.NewGuid();
                    entity.PolicyNumber = model.PolicyNo;
                    entity.FrequencyType_Old = model.FrequencyType_Old.ToString();
                    entity.FrequencyType_New = model.FrequencyType_New.ToString();
                    entity.Amount_Old = model.Amount_Old;
                    entity.Amount_New = model.Amount_New;
                    ...
                    unitOfWork.GetRepository<Entities.ServicePaymentFrequency>().Add(entity);
                    ...
                    serviceMain.ServiceType = EnumServiceType.PaymentFrequency.ToString();
                    ...
                    ILPaymentFrequencyRequest iLPaymentFrequencyRequest = new ILPaymentFrequencyRequest();

                    // string frequencyType = "";
                    // if(model.FrequencyType_New == EnumPaymentFrequency.Monthly)
                    // {
                    //     frequencyType = "12";
                    // }
                    // else if(model.FrequencyType_New == EnumPaymentFrequency.Quarterly)
                    // {
                    //     frequencyType = "04";
                    // }
                    // else if(model.FrequencyType_New == EnumPaymentFrequency.SemiAnnually)
                    // {
                    //     frequencyType = "02";
                    // }
                    //  else if(model.FrequencyType_New == EnumPaymentFrequency.Annually)
                    // {
                    //     frequencyType = "01";
                    // }
                    string frequencyType = model.FrequencyType_New.ToString();

                    iLPaymentFrequencyRequest.frequency = frequencyType;
                    iLPaymentFrequencyRequest.policyNumber = model.PolicyNo;
                    iLPaymentFrequencyRequest.requestType = "CPF";
                    iLPaymentFrequencyRequest.requestId = entity.ID.ToString();
                    
                    entity.ILRequest = JsonConvert.SerializeObject(iLPaymentFrequencyRequest);
                    entity.ILRequestOn = Utils.GetDefaultDate();

                    //AIA IL
                    CommonRegisterResponse ilResponse = aiaILApiService.PaymentFrequencySubmission(iLPaymentFrequencyRequest, out string systemError, out string SerializeModel);

                    entity.ILResponse = systemError;
                    entity.ILResponseOn = Utils.GetDefaultDate();
                    unitOfWork.GetRepository<Entities.ServicePaymentFrequency>().Update(entity);
                    unitOfWork.SaveChanges();
                    ...
                    try
                    {
                        ...
                        _ = notificationService.SendServicingNoti(memberID ?? new Guid(), entity.ID, EnumServicingStatus.Received, EnumServiceType.PaymentFrequency, model.PolicyNo);
        ...
        private async Task RunBackgroundCodeAsync(ServicePaymentFrequency entity, Client client, Policy policy, PaymentFrequencyRequest model, Guid? memberID, string ServicingEmail, string pdfFileName, byte[] pdfData, CommonRegisterResponse ilResponse)
        {
            ...
            try
            {
                ...
                string subject = $"{entity.PolicyNumber}/Payment Frequency/{client.Name}";
        ...
        public async Task<string> CreateCaseToCRM(Client client, ServicePaymentFrequency entity, Policy policy)
        {
            try
            {
                ServicePaymentFrequency data = unitOfWork.GetRepository<ServicePaymentFrequency>().Query(x=> x.ID == entity.ID).FirstOrDefault();
                ...
                crmModel.RequestInfo.CaseCategory = "CC007";
                ...
                MobileErrorLog("PaymentFrequencyRepository aiaCrmApiService => CreateCase", "Request"
                    , JsonConvert.SerializeObject(crmModel), "v1/claim/claim");

                var crmResponse = await aiaCrmApiService.CreateCase(crmModel);


                MobileErrorLog("PaymentFrequencyRepository aiaCrmApiService => CreateCase", "Response"
                ...
                {
                    MobileErrorLog("update PaymentFrequencyRepository tran ex after Crm call", ex.Message
        ...
        private async Task<byte[]> GenerateServicingPdf(ServicePaymentFrequency data, string pdfFileName, string holderName, string policyNumber, string policyStatus, string clientNo)
        {
            var path = Path.Combine(
                    this.environment.ContentRootPath, "email_templates/", "payment_frequency_request.html");

            var htmlData = File.ReadAllText(path);

            htmlData = htmlData.Replace("{{Title}}", "Request for Alteration Billing");
            htmlData = htmlData.Replace("{{HolderName}}", holderName);
            htmlData = htmlData.Replace("{{ClientNo}}", clientNo);

            htmlData = htmlData.Replace("{{ChangeType}}", "Payment Frequency Information");
            ...
            htmlData = htmlData.Replace("{{FormID}}", "POSBLM1");
            ...
            string pdftemplatedata = @"<tr class='fieldTable' style='height: 40px;'>
                                        <td class='fieldTable' style='width: 30%;'><b>{{ChangeType}}</b></td>
                                        <td class='fieldTable' style='width: 30%;'><b>{{OldInformation}}</b></td>
                                        <td class='fieldTable' style='width: 30%;'><b>{{NewInformation}}</b></td>
                                    </tr>";
            pdftemplatedata = pdftemplatedata.Replace("{{ChangeType}}","Payment Frequency Information");
            pdftemplatedata = pdftemplatedata.Replace("{{OldInformation}}",Utils.GetPaymentFrequency(data.FrequencyType_Old));
            pdftemplatedata = pdftemplatedata.Replace("{{NewInformation}}",Utils.GetPaymentFrequency(data.FrequencyType_New));

            htmlData = htmlData.Replace("{{update_fields}}", pdftemplatedata);
            try
            {
        ...
        private async Task<string> UpadeGeneratePdfToCMS(byte[] pdfData, string policyNo, string fileName, ServicePaymentFrequency model)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"PaymentFrequency => azureStorage.UploadBase64Async => {JsonConvert.SerializeObject(azureUpload)}");
                ...
                await unitOfWork.GetRepository<Entities.ServiceMainDoc>().AddAsync(new ServiceMainDoc
                {
                    ...
                    FormId = GetServicingFormId(EnumServiceType.PaymentFrequency, true),
                    DocName = fileName,
                    DocType = "pdf",
