...
{
    public interface ILapseReinstatementRepository
    {
        Task<ResponseModel<ServicingResponseModel>> Submit(ServiceCommonPartOne model);
    }
    public class LapseReinstatementRepository : BaseRepository, ILapseReinstatementRepository
    {
        #region "const"
        ...
        public LapseReinstatementRepository(IHttpContextAccessor httpContext, IAzureStorageService azureStorage, IErrorCodeProvider errorCodeProvider,
        ...
        public async Task<ResponseModel<ServicingResponseModel>> Submit(ServiceCommonPartOne model)
        {
            try
            {
                ...
                {
                    ServiceLapseReinstatement entity = new ServiceLapseReinstatement();
                    ...
                    unitOfWork.GetRepository<Entities.ServiceLapseReinstatement>().Add(entity);
                    ...
                    serviceMain.ServiceType = EnumServiceType.LapseReinstatement.ToString();
                    ...
                    try
                    {
                        ...
                        _ = notificationService.SendServicingNoti(memberID ?? new Guid(), entity.ID, EnumServicingStatus.Received, EnumServiceType.LapseReinstatement, model.PolicyNo);
                    }
        ...
        private async Task RunBackgroundCodeAsync(ServiceLapseReinstatement entity, Client client, Policy policy, ServiceCommonPartOne model, Guid? memberID, string ServicingEmail, string pdfFileName, byte[] pdfData)
        {
            ...
            try
            {
                ...
                string subject = $"{entity.PolicyNumber}/Lapse Reinstatement/{client.Name}";
        ...
        public async Task<(List<byte[]>, List<string>)> UploadDocToCMS(ServiceCommonPartOne model, Policy policy, ServiceLapseReinstatement entity)
        {
            try
            {
                ...
                foreach (var doc in model.DocNameList)
                {
                    try
                    {
                        ...
                        ServiceLapseReinstatementDoc docData = new ServiceLapseReinstatementDoc();

                        docData.ServiceLapseReinstatementID = entity.ID;
                        ...
                        unitOfWork.GetRepository<Entities.ServiceLapseReinstatementDoc>().Add(docData);
                        unitOfWork.SaveChanges();
                        ...
                        try
                        {
                            unitOfWork.GetRepository<Entities.ServiceMainDoc>().Add(new ServiceMainDoc
                            {
                                ...
                                FormId = GetServicingFormId(EnumServiceType.LapseReinstatement),
                                DocName = doc,
                                DocType = format,
        ...
        public async Task<string> CreateCaseToCRM(Client client, ServiceLapseReinstatement entity, Policy policy)
        {
            try
            {
                ServiceLapseReinstatement data = unitOfWork.GetRepository<ServiceLapseReinstatement>().Query(x => x.ID == entity.ID).FirstOrDefault();
                ...
                crmModel.RequestInfo.CaseCategory = "CC012";
        ...
        private async Task<byte[]> GenerateServicingPdf(ServiceLapseReinstatement data, string pdfFileName, string holderName, string policyNumber, string policyStatus, string clientNo)
        {
            var path = Path.Combine(
            ...
            htmlData = htmlData.Replace("{{Title}}", "Request for Lapse Reinstatement");
            htmlData = htmlData.Replace("{{HolderName}}", holderName);
            htmlData = htmlData.Replace("{{ClientNo}}", clientNo);

            htmlData = htmlData.Replace("{{ChangeType}}", "Lapse Reinstatement");
        ...
        private async Task<string> UpadeGeneratePdfToCMS(byte[] pdfData, string policyNo, string fileName, ServiceLapseReinstatement model)
        {
            var upload = new UploadBase64Request();
            ...
            try
            {
                ...
                Console.WriteLine($"Lapse => azureStorage.UploadBase64Async => {JsonConvert.SerializeObject(azureUpload)}");
                ...
                await unitOfWork.GetRepository<Entities.ServiceMainDoc>().AddAsync(new ServiceMainDoc
                {
                    ...
                    CmsResponse = JsonConvert.SerializeObject(uploadResult),
                    FormId = GetServicingFormId(EnumServiceType.LapseReinstatement, true),
                    DocName = fileName,
                    DocType = "pdf",
                    UploadStatus = uploadResult?.msg,
                });
