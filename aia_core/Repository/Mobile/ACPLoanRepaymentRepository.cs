...
{
    public interface IACPLoanRepaymentRepository
    {
        Task<ResponseModel<ServicingResponseModel>> Submit(ServiceCommonPartOne model);
    }
    public class ACPLoanRepaymentRepository : BaseRepository, IACPLoanRepaymentRepository
    {
        #region "const"
        ...
        public ACPLoanRepaymentRepository(IHttpContextAccessor httpContext, IAzureStorageService azureStorage, IErrorCodeProvider errorCodeProvider,
        ...
        public async Task<ResponseModel<ServicingResponseModel>> Submit(ServiceCommonPartOne model)
        {
            try
            {
                ...
                {
                    ServiceACPLoanRepayment entity = new ServiceACPLoanRepayment();
                    ...
                    unitOfWork.GetRepository<Entities.ServiceACPLoanRepayment>().Add(entity);
                    ...
                    try
                    {
                        ...
                        _ = notificationService.SendServicingNoti(memberID ?? new Guid(), entity.ID, EnumServicingStatus.Received, EnumServiceType.AcpLoanRepayment, model.PolicyNo);
                    ...
                    /*await */ _ = UpadeGeneratePdfToCMS(pdfData, entity.PolicyNumber, pdfFileName, entity);
        ...
        private async Task RunBackgroundCodeAsync(ServiceACPLoanRepayment entity, Client client, Policy policy, ServiceCommonPartOne model, Guid? memberID, string ServicingEmail, string pdfFileName, byte[] pdfData)
        {
            ...
            try
            {
                ...
                string subject = $"{entity.PolicyNumber}/ACP Loan Repayment/{client.Name}";
        ...
        public async Task<(List<byte[]>, List<string>)> UploadDocToCMS(ServiceCommonPartOne model, Policy policy, ServiceACPLoanRepayment entity)
        {
            try
            {
                ...
                foreach (var doc in model.DocNameList)
                {
                    try
                    {
                        ...
                        ServiceACPLoanRepaymentDoc docData = new ServiceACPLoanRepaymentDoc();

                        docData.ServiceACPLoanRepaymentID = entity.ID;
                        ...
                        unitOfWork.GetRepository<Entities.ServiceACPLoanRepaymentDoc>().Add(docData);
                        unitOfWork.SaveChanges();
                        ...
                        try
                        {
                            unitOfWork.GetRepository<Entities.ServiceMainDoc>().Add(new ServiceMainDoc
                            {
                                ...
                                FormId = GetServicingFormId(EnumServiceType.AcpLoanRepayment),
                                DocName = doc,
                                DocType = format,
                    ...
                    {
                        MobileErrorLog($"ServiceACPLoanRepayment => UploadBase64", ex.Message, JsonConvert.SerializeObject(ex), httpContext?.HttpContext.Request.Path);
                    }
        ...
        public async Task<string> CreateCaseToCRM(Client client, ServiceACPLoanRepayment entity, Policy policy)
        {
            try
            {
                ServiceACPLoanRepayment data = unitOfWork.GetRepository<ServiceACPLoanRepayment>().Query(x=> x.ID == entity.ID).FirstOrDefault();
        ...
        private async Task<byte[]> GenerateServicingPdf(ServiceACPLoanRepayment data, string pdfFileName, string holderName, string policyNumber, string policyStatus, string clientNo)
        {
            var path = Path.Combine(
            ...
            htmlData = htmlData.Replace("{{Title}}", "Request for ACP Loan Repayment");
            htmlData = htmlData.Replace("{{HolderName}}", holderName);
            htmlData = htmlData.Replace("{{ClientNo}}", clientNo);

            htmlData = htmlData.Replace("{{ChangeType}}", "ACP Loan Repayment");
        ...
        private async Task<string> UpadeGeneratePdfToCMS(byte[] pdfData, string policyNo, string fileName, ServiceACPLoanRepayment model)
        {
            var upload = new UploadBase64Request();
            ...
            try
            {
                ...
                Console.WriteLine($"azureStorage.UploadBase64Async => {JsonConvert.SerializeObject(azureUpload)}");
                ...
                await unitOfWork.GetRepository<Entities.ServiceMainDoc>().AddAsync(new ServiceMainDoc
                {
                    ...
                    CmsResponse = JsonConvert.SerializeObject(uploadResult),
                    FormId = GetServicingFormId(EnumServiceType.AcpLoanRepayment, true),
                    DocName = fileName,
                    DocType = "pdf",
                    UploadStatus = uploadResult?.msg,
                });
