...
{
    public interface IAdhocTopupRepository
    {
        Task<ResponseModel<ServicingResponseModel>> Submit(ServiceCommonPartOne model);
    }
    public class AdhocTopupRepository : BaseRepository, IAdhocTopupRepository
    {
        #region "const"
        ...
        public AdhocTopupRepository(IHttpContextAccessor httpContext, IAzureStorageService azureStorage, IErrorCodeProvider errorCodeProvider,
        ...
        public async Task<ResponseModel<ServicingResponseModel>> Submit(ServiceCommonPartOne model)
        {
            try
            {
                ...
                {
                    ServiceAdhocTopup entity = new ServiceAdhocTopup();
                    ...
                    unitOfWork.GetRepository<Entities.ServiceAdhocTopup>().Add(entity);
                    ...
                    ILAdhocTopupRequest iLAdhocTopupRequest = new ILAdhocTopupRequest();
                    iLAdhocTopupRequest.amount = model.Amount;
                    iLAdhocTopupRequest.policyNumber = model.PolicyNo;
                    iLAdhocTopupRequest.requestId = entity.ID.ToString();
                    if(crmResponse.Code== "200")
                    {
                        iLAdhocTopupRequest.incidentID = crmResponse.Data;  
                    }
                    else
                    {
                        iLAdhocTopupRequest.incidentID = "";
                    }

                    entity.ILRequest = JsonConvert.SerializeObject(iLAdhocTopupRequest);
                    entity.ILRequestOn = Utils.GetDefaultDate();

                    //AIA IL
                    CommonRegisterResponse ilResponse = await aiaILApiService.AdhocTopSubmission(iLAdhocTopupRequest);

                    entity.ILResponse = JsonConvert.SerializeObject(ilResponse);
                    entity.ILResponseOn = Utils.GetDefaultDate();
                    unitOfWork.GetRepository<Entities.ServiceAdhocTopup>().Update(entity);
                    unitOfWork.SaveChanges();
                    ...
                    try
                    {
                        ...
                        _ = notificationService.SendServicingNoti(memberID ?? new Guid(), entity.ID, EnumServicingStatus.Received, EnumServiceType.AdHocTopup, model.PolicyNo);
        ...
        private async Task RunBackgroundCodeAsync(ServiceAdhocTopup entity, Client client, Policy policy, ServiceCommonPartOne model, Guid? memberID, string ServicingEmail, string pdfFileName, byte[] pdfData, CommonRegisterResponse ilResponse)
        {
            ...
            try
            {
                ...
                string subject = $"{entity.PolicyNumber}/Adhoc Topup/{client.Name}";
        ...
        public async Task<(List<byte[]>, List<string>)> UploadDocToCMS(ServiceCommonPartOne model, Policy policy, ServiceAdhocTopup entity)
        {
            try
            {
                ...
                foreach (var doc in model.DocNameList)
                {
                    try
                    {
                        ...
                        ServiceAdhocTopupDoc docData = new ServiceAdhocTopupDoc();

                        docData.ServiceAdhocTopupID = entity.ID;
                        ...
                        unitOfWork.GetRepository<Entities.ServiceAdhocTopupDoc>().Add(docData);
                        unitOfWork.SaveChanges();
                        ...
                        try
                        {
                            unitOfWork.GetRepository<Entities.ServiceMainDoc>().Add(new ServiceMainDoc
                            {
                                ...
                                FormId = GetServicingFormId(EnumServiceType.AdHocTopup),
                                DocName = doc,
                                DocType = format,
                    ...
                    {
                        MobileErrorLog($"ServiceAdhocTopup => UploadBase64", ex.Message, JsonConvert.SerializeObject(ex), httpContext?.HttpContext.Request.Path);
                    }
        ...
        public async Task<CaseResponse> CreateCaseToCRM(Client client, ServiceAdhocTopup entity, Policy policy)
        {
            try
            {
                ServiceAdhocTopup data = unitOfWork.GetRepository<ServiceAdhocTopup>().Query(x=> x.ID == entity.ID).FirstOrDefault();
                ...
                crmModel.RequestInfo.CaseCategory = "CC013";
        ...
        private async Task<byte[]> GenerateServicingPdf(ServiceAdhocTopup data, string pdfFileName, string holderName, string policyNumber, string policyStatus, string clientNo)
        {
            var path = Path.Combine(
            ...
            htmlData = htmlData.Replace("{{Title}}", "Request for Adhoc Topup");
            htmlData = htmlData.Replace("{{HolderName}}", holderName);
            htmlData = htmlData.Replace("{{ClientNo}}", clientNo);

            htmlData = htmlData.Replace("{{ChangeType}}", "Adhoc Topup");
        ...
        private async Task<string> UpadeGeneratePdfToCMS(byte[] pdfData, string policyNo, string fileName, ServiceAdhocTopup model)
        {
            var upload = new UploadBase64Request();
            ...
            try
            {
                ...
                Console.WriteLine($"AdhocTopup => azureStorage.UploadBase64Async => {JsonConvert.SerializeObject(azureUpload)}");
                ...
                await unitOfWork.GetRepository<Entities.ServiceMainDoc>().AddAsync(new ServiceMainDoc
                {
                    ...
                    CmsResponse = JsonConvert.SerializeObject(uploadResult),
                    FormId = GetServicingFormId(EnumServiceType.AdHocTopup, true),
                    DocName = fileName,
                    DocType = "pdf",
                    UploadStatus = uploadResult?.msg,
                });
