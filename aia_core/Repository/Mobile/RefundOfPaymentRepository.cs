...
{
    public interface IRefundOfPaymentRepository
    {
        Task<ResponseModel<ServicingResponseModel>> Submit(ServiceWithdrawRequest model);
    }
    public class RefundOfPaymentRepository : BaseRepository, IRefundOfPaymentRepository
    {
        #region "const"
        ...
        public RefundOfPaymentRepository(IHttpContextAccessor httpContext, IAzureStorageService azureStorage, IErrorCodeProvider errorCodeProvider,
        ...
        public async Task<ResponseModel<ServicingResponseModel>> Submit(ServiceWithdrawRequest model)
        {
            try
            {
                    ...
                    && claimOtp?.OtpExpiry > defaultDate) || model.IsSkipOtpValidation)
                {
                    ...
                    var ilBankCode = unitOfWork.GetRepository<Entities.Bank>()
                    .Query(x => x.BankCode == model.BankInfo.BankCode && x.IsDelete == false)
                    .Select(x => x.IlBankCode)
                    .FirstOrDefault();

                    ServiceRefundOfPayment entity = new ServiceRefundOfPayment();
                    ...
                    unitOfWork.GetRepository<Entities.ServiceRefundOfPayment>().Add(entity);
                    ...
                    ILPaymentRefundRequest iLPaymentRefundRequest = new ILPaymentRefundRequest();
                    iLPaymentRefundRequest.amount = model.Amount;
                    iLPaymentRefundRequest.policyNumber = model.PolicyNo;
                    iLPaymentRefundRequest.requestId = entity.ID.ToString();
                    iLPaymentRefundRequest.paymentType = "F";

                    if(crmResponse.Code== "200")
                    {
                        iLPaymentRefundRequest.incidentID = crmResponse.Data;  
                    }
                    else
                    {
                        iLPaymentRefundRequest.incidentID = "";
                    }

                    iLPaymentRefundRequest.bankCode = ilBankCode; //model.BankInfo.BankCode;

                    entity.ILRequest = JsonConvert.SerializeObject(iLPaymentRefundRequest);
                    entity.ILRequestOn = Utils.GetDefaultDate();

                    //AIA IL
                    CommonRegisterResponse ilResponse = await aiaILApiService.RefundPaymentSubmission(iLPaymentRefundRequest);

                    entity.ILResponse = JsonConvert.SerializeObject(ilResponse);
                    entity.ILResponseOn = Utils.GetDefaultDate();
                    unitOfWork.GetRepository<Entities.ServiceRefundOfPayment>().Update(entity);
                    unitOfWork.SaveChanges();
                    ...
                    try
                    {
                        ...
                        _ = notificationService.SendServicingNoti(memberID ?? new Guid(), entity.ID, EnumServicingStatus.Received, EnumServiceType.RefundOfPayment, model.PolicyNo);
            ...
            {
                MobileErrorLog("RefundofPayment", ex.Message, JsonConvert.SerializeObject(ex), httpContext?.HttpContext.Request.Path);

                return errorCodeProvider.GetResponseModel<ServicingResponseModel>(ErrorCode.E500);
            }
        }

        private async Task RunBackgroundCodeAsync(ServiceRefundOfPayment entity, Client client, Policy policy, ServiceWithdrawRequest model, Guid? memberID, string ServicingEmail, string pdfFileName, byte[] pdfData, CommonRegisterResponse ilResponse)
        {
            ...
            try
            {
                ...
                {
                    MobileErrorLog("imageByteList Count RefundOfPayment", imageByteList.Count.ToString(), "", httpContext?.HttpContext.Request.Path);
                ...
                string subject = $"{entity.PolicyNumber}/Refund Of Payment/{client.Name}";
            ...
            catch (System.Exception ex)
            {
                ...
                MobileErrorLog("RefundofPayment > Servicing Request Email Error", ex.Message, JsonConvert.SerializeObject(ex), httpContext?.HttpContext.Request.Path);
        ...
        public async Task<(List<byte[]>, List<string>)> UploadDocToCMS(ServiceWithdrawRequest model, Policy policy, ServiceRefundOfPayment entity)
        {
            try
            {
                ...
                foreach (var doc in model.DocNameList)
                {
                    try
                    {
                        ...
                        ServiceRefundOfPaymentDoc docData = new ServiceRefundOfPaymentDoc();

                        docData.ServiceRefundOfPaymentID = entity.ID;
                        ...
                        unitOfWork.GetRepository<Entities.ServiceRefundOfPaymentDoc>().Add(docData);
                        unitOfWork.SaveChanges();
                        ...
                        try
                        {
                            unitOfWork.GetRepository<Entities.ServiceMainDoc>().Add(new ServiceMainDoc
                            {
                                ...
                                FormId = GetServicingFormId(EnumServiceType.RefundOfPayment),
                                DocName = doc,
                                DocType = format,
                    ...
                    {
                        MobileErrorLog($"ServiceRefundOfPayment => UploadBase64", ex.Message, JsonConvert.SerializeObject(ex), httpContext?.HttpContext.Request.Path);
                    }
        ...
        public async Task<CaseResponse> CreateCaseToCRM(Client client, ServiceRefundOfPayment entity, Policy policy)
        {
            try
            {
                ServiceRefundOfPayment data = unitOfWork.GetRepository<ServiceRefundOfPayment>().Query(x=> x.ID == entity.ID).FirstOrDefault();
                ...
                crmModel.RequestInfo.CaseCategory = "CC004";
                ...
                MobileErrorLog("ServiceRefundOfPayment aiaCrmApiService => CreateCase", "Request"
                    , JsonConvert.SerializeObject(crmModel), "v1/claim/claim");

                CaseResponse crmResponse = await aiaCrmApiService.CreateCase(crmModel);


                MobileErrorLog("ServiceRefundOfPayment aiaCrmApiService => CreateCase", "Response"
                ...
                {
                    MobileErrorLog("update ServiceRefundOfPayment tran ex after Crm call", ex.Message
        ...
        private async Task<byte[]> GenerateServicingPdf(ServiceRefundOfPayment data, string pdfFileName, string holderName, string policyNumber, string policyStatus, string clientNo)
        {
            var path = Path.Combine(
            ...
            htmlData = htmlData.Replace("{{Title}}", "Request for Refund Of Payment");
            htmlData = htmlData.Replace("{{HolderName}}", holderName);
            htmlData = htmlData.Replace("{{ClientNo}}", clientNo);

            htmlData = htmlData.Replace("{{ChangeType}}", "Refund Of Payment");
        ...
        private async Task<string> UpadeGeneratePdfToCMS(byte[] pdfData, string policyNo, string fileName, ServiceRefundOfPayment model)
        {
            var upload = new UploadBase64Request();
            ...
            try
            {
                ...
                Console.WriteLine($"RefundOfPayment => azureStorage.UploadBase64Async => {JsonConvert.SerializeObject(azureUpload)}");
                ...
                { 
                    ...
                    CmsResponse = JsonConvert.SerializeObject(uploadResult),
                    FormId = GetServicingFormId(EnumServiceType.RefundOfPayment, true),
                    DocName = fileName,
                    DocType = "pdf",
                    UploadStatus = uploadResult?.msg,                    
                });
