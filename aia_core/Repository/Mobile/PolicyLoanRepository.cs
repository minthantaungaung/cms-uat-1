...
{
    public interface IPolicyLoanRepository
    {
        Task<ResponseModel<ServicingResponseModel>> Submit(ServiceWithdrawRequest model);
    }
    public class PolicyLoanRepository : BaseRepository, IPolicyLoanRepository
    {
        #region "const"
        ...
        public PolicyLoanRepository(IHttpContextAccessor httpContext, IAzureStorageService azureStorage, IErrorCodeProvider errorCodeProvider,
        ...
        public async Task<ResponseModel<ServicingResponseModel>> Submit(ServiceWithdrawRequest model)
        {
            try
            {
                ...
                {
                    ServicePolicyLoan entity = new ServicePolicyLoan();
                    ...
                    unitOfWork.GetRepository<Entities.ServicePolicyLoan>().Add(entity);
                    ...
                    try
                    {
                        ...
                        _ = notificationService.SendServicingNoti(memberID ?? new Guid(), entity.ID, EnumServicingStatus.Received, EnumServiceType.PolicyLoan, model.PolicyNo);
        ...
        private async Task RunBackgroundCodeAsync(ServicePolicyLoan entity, Client client, Policy policy, ServiceWithdrawRequest model, Guid? memberID, string ServicingEmail, string pdfFileName, byte[] pdfData)
        {
            ...
            try
            {
                ...
                MobileErrorLog("attachments Count ServicePolicyLoan", attachments.Count.ToString(), "", httpContext?.HttpContext.Request.Path);


                string subject = $"{entity.PolicyNumber}/Policy Loan/{client.Name}";
        ...
        public async Task<(List<byte[]>, List<string>)> UploadDocToCMS(ServiceWithdrawRequest model, Policy policy, ServicePolicyLoan entity)
        {
            try
            {
                ...
                foreach (var doc in model.DocNameList)
                {
                    try
                    {
                        ...
                        ServicePolicyLoanDoc docData = new ServicePolicyLoanDoc();

                        docData.ServicePolicyLoanID = entity.ID;
                        ...
                        unitOfWork.GetRepository<Entities.ServicePolicyLoanDoc>().Add(docData);
                        unitOfWork.SaveChanges();
                        ...
                        try
                        {
                            unitOfWork.GetRepository<Entities.ServiceMainDoc>().Add(new ServiceMainDoc
                            {
                                ...
                                FormId = GetServicingFormId(EnumServiceType.PolicyLoan),
                                DocName = doc,
                                DocType = format,
                    ...
                    {
                        MobileErrorLog($"ServicePolicyLoan => UploadBase64", ex.Message, JsonConvert.SerializeObject(ex), httpContext?.HttpContext.Request.Path);
                    }
        ...
        public async Task<string> CreateCaseToCRM(Client client, ServicePolicyLoan entity, Policy policy)
        {
            try
            {
                ServicePolicyLoan data = unitOfWork.GetRepository<ServicePolicyLoan>().Query(x=> x.ID == entity.ID).FirstOrDefault();
                ...
                crmModel.RequestInfo.CaseCategory = "CC002";
                ...
                MobileErrorLog("ServicePolicyLoan aiaCrmApiService => CreateCase", "Request"
                    , JsonConvert.SerializeObject(crmModel), "v1/claim/claim");

                var crmResponse = await aiaCrmApiService.CreateCase(crmModel);


                MobileErrorLog("ServicePolicyLoan aiaCrmApiService => CreateCase", "Response"
                ...
                {
                    MobileErrorLog("update ServicePolicyLoan tran ex after Crm call", ex.Message
        ...
        private async Task<byte[]> GenerateServicingPdf(ServicePolicyLoan data, string pdfFileName, string holderName, string policyNumber, string policyStatus, string clientNo)
        {
            var path = Path.Combine(
            ...
            htmlData = htmlData.Replace("{{Title}}", "Request for Policy Loan");
            htmlData = htmlData.Replace("{{HolderName}}", holderName);
            htmlData = htmlData.Replace("{{ClientNo}}", clientNo);

            htmlData = htmlData.Replace("{{ChangeType}}", "Policy Loan");
        ...
        private async Task<string> UpadeGeneratePdfToCMS(byte[] pdfData, string policyNo, string fileName, ServicePolicyLoan model)
        {
            var upload = new UploadBase64Request();
            ...
            try
            {
                ...
                Console.WriteLine($"PolicyLoan => azureStorage.UploadBase64Async => {JsonConvert.SerializeObject(azureUpload)}");
                ...
                await unitOfWork.GetRepository<Entities.ServiceMainDoc>().AddAsync(new ServiceMainDoc
                {
                    ...
                    CmsResponse = JsonConvert.SerializeObject(uploadResult),
                    FormId = GetServicingFormId(EnumServiceType.PolicyLoan, true),
                    DocName = fileName,
                    DocType = "pdf",
                    UploadStatus = uploadResult?.msg,
                });
